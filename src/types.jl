...
Representation of an R symbolic expression record (`SEXPREC` in the R API).

A pointer to a `Sxp`,  `Ptr{S<:Sxp}`, is called an `SEXP` in the R API.
...
"""
const SxpPtrInfo = UInt64 # sxpinfo_struct
...
Representation of R `NULL` value (R type tag 0)
...
Representation of R pairs (cons) list cell (R type tag 2)
...
Representation of R function closure (R type tag 3)
...
Representation of R environment (R type tag 4)
...
Representation of R promise (R type tag 5)
...
Representation of R function call (R type tag 6)
...
Representation of R special function (R type tag 7)
...
Representation of R built-in function (R type tag 8)
...
Representation of R character string (R type tag 9)

A character vector in R is a `StrSxp`.
A `CharSxp` is a single character string which is never visible in the R REPL.
...
# Definition of SymSxp must occur after CharSxp because the `name` field is a CharSxp
"""
    SymSxp <: Sxp

Representation of R symbol (R type tag 1)
...
Representation of R logical vector (R type tag 10)

For this an other `VectorNumericSxp` types, `dataptr` returns a pointer to the vector's contents
...
# There are no Sxp types with type tags 11 and 12.
# These tags were at one time used for Factor and OrderedFactor but now these are represented differently.

""""
    IntSxp <: VectorNumericSxp

Representation of R integer vector (R type tag 13)
...
Representation of R real (numeric) vector (R type tag 14)
...
Representation of R complex vector (R type tag 15)
...
Representation of R vector of character strings (R type tag 16)
...
Representation of R dot-dot-dot object (R type tag 17)
...
Representation of R "any" object (comparable to `Ref{Any}`) (R type tag 18)
...
Representation of R list (i.e. `Array{Any,1}`) (R type tag 19)
...
Representation of R expression vector (R type tag 20)
...
Representation of R byte code (R type tag 21)
...
Representation of R external pointer (R type tag 22)
...
Representation of R weak reference (R type tag 23)
...
Representation of R byte vector (R type tag 24)
...
Representation of R S4 object (R type tag 25)
...
An `RObject` is a Julia wrapper for an R object (known as an "S-expression", i.e. `SEXPREC`).
...
Examples:
- `Rconversion.RClass{:logical}`
- `Rconversion.RClass{:integer}`
- `Rconversion.RClass{:numeric}`
- `Rconversion.RClass{:character}`
"""
...
but more flexible.
"""
preserve(p::Ptr{S}) where {S<:Sxp} = ccall(:R_PreserveObject, Nothing, (Ptr{S},), p)
...
"""
release(p::Ptr{S}) where {S<:Sxp} = ccall(:R_ReleaseObject, Nothing, (Ptr{S},), p)
...
"""
protect(p::Ptr{S}) where {S<:Sxp} = ccall(:Rf_protect, Ptr{S}, (Ptr{S},), p)
...
"""
unprotect(n::Integer) = ccall(:Rf_unprotect, Nothing, (Cint,), n)
