...
function rcopy(::Type{T}, s::Ptr{SymSxp}) where {T<:Union{Symbol,AbstractString}}
    rcopy(T, ccall(:PRINTNAME, Ptr{CharSxp}, (Ptr{SymSxp},), s))
end

# CharSxp
function rcopy(::Type{T}, s::Ptr{CharSxp}) where {T<:AbstractString}
    c = ccall(:R_CHAR, Ptr{Cchar}, (Ptr{CharSxp},), s)
...
sexp(::Type{RClass{:logical}}, v::Cint) = ccall(:Rf_ScalarLogical, Ptr{LglSxp}, (Cint,), v)
...
# String
sexp(::Type{SymSxp}, s::AbstractString) = ccall(:Rf_install, Ptr{SymSxp}, (Ptr{UInt8},), s)
sexp(::Type{CharSxp}, st::String) = ccall(
    :Rf_mkCharLenCE,
    Ptr{CharSxp},
    (Ptr{UInt8}, Cint, Cint),
    st,
...
sexp(::Type{RClass{:character}}, s::Ptr{CharSxp}) =
    ccall(:Rf_ScalarString, Ptr{StrSxp}, (Ptr{CharSxp},), s)
sexp(::Type{RClass{:character}}, st::AbstractString) =
    sexp(RClass{:character}, sexp(CharSxp, st))
