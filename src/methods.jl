...
"""
length(s::Ptr{S}) where {S<:Sxp} = Int(ccall(:Rf_xlength, Cptrdiff_t, (Ptr{S},), s))
...
for sym in (
    ...
    @eval begin
        $sym(s::Ptr{S}) where {S<:Sxp} =
            ccall($(string("Rf_", sym)), Bool, (Ptr{SxpPtrInfo},), s)
...
for (S, J) in (
    ...
    @eval dataptr(s::Ptr{$S}) =
        convert(Ptr{eltype($S)}, ccall($J, Ptr{Cvoid}, (Ptr{UnknownSxp},), s))
end
...
setindex!(r::RObject{S}, value, keys...) where {S<:VectorSxp} =
    setindex!(sexp(r), value, keys...)
setindex!(r::RObject{S}, ::Missing, keys...) where {S<:VectorSxp} =
    setindex!(sexp(r), naeltype(S), keys...)
...
"""
    Get an element of a VectorSxp by a label.
"""
...
"""
    Set an element of a VectorSxp by a label.
"""
function setindex!(
    s::Ptr{S},
    value::Ptr{T},
    label::AbstractString,
) where {S<:VectorSxp,T<:Sxp}
    ls = getnames(s)
    for (i, l) in enumerate(ls)
        if rcopy(String, l) == label
            s[i] = value
...
setindex!(s::Ptr{S}, value, label::AbstractString) where {S<:VectorSxp} =
    setindex!(s, sexp(value), label)
setindex!(s::Ptr{S}, value, label::Symbol) where {S<:VectorSxp} =
    setindex!(s, value, string(label))


# VectorAtomicSxp
getindex(s::Ptr{S}, I::Integer) where {S<:VectorAtomicSxp} = getindex(unsafe_vec(s), I)
getindex(s::Ptr{S}, I::Integer...) where {S<:VectorAtomicSxp} =
    getindex(unsafe_array(s), I...)
getindex(s::Ptr{S}, I::AbstractVector) where {S<:VectorAtomicSxp} =
    getindex(unsafe_vec(s), I)
...
function getindex(s::Ptr{StrSxp}, key::Integer)
    c = ccall(:STRING_ELT, Ptr{CharSxp}, (Ptr{StrSxp}, Cint), s, key-1)
end
...
cdr(s::Ptr{S}) where {S<:PairListSxp} = sexp(ccall(:CDR, Ptr{UnknownSxp}, (Ptr{S},), s))
...
IteratorEltype(x::Pairs{K,V,I,RObject{S}}) where {K,V,I,S<:PairListSxp} =
    Base.EltypeUnknown()
@inline iterate(x::Pairs{K,V,I,RObject{S}}) where {K,V,I,S<:PairListSxp} =
    iterate(x, values(x).p)
...
setindex!(s::Ptr{S}, value, label::AbstractString) where {S<:PairListSxp} =
    setindex!(s, sexp(value), label)
setindex!(s::Ptr{S}, value, label::Symbol) where {S<:PairListSxp} =
    setindex!(s, value, string(label))
...
function getindex(s::Ptr{S4Sxp}, sym::Ptr{SymSxp})
    if ccall(:R_has_slot, Int, (Ptr{S4Sxp}, Ptr{SymSxp}), s, sym) == 1
        return sexp(ccall(:R_do_slot, Ptr{UnknownSxp}, (Ptr{S4Sxp}, Ptr{SymSxp}), s, sym))
    else
        throw(BoundsError())
    end
end
...
function setindex!(s::Ptr{S4Sxp}, value::Ptr{T}, sym::Ptr{SymSxp}) where {T<:Sxp}
    ...
    try
        t = rcall_p(
            findNamespace("methods")[:checkSlotAssignment],
            s,
            rcopy(String, sym),
            value,
        )
...
allocArray(::Type{S}, n1::Integer, n2::Integer, n3::Integer) where {S<:Sxp} =
    ccall(:Rf_alloc3DArray, Ptr{S}, (Cint, Cint, Cint, Cint), sexpnum(S), n1, n2, n3)

function allocArray(::Type{S}, dims::Integer...) where {S<:Sxp}
    sdims = sexp(RClass{:integer}, [dims...])
    ccall(:Rf_allocArray, Ptr{S}, (Cint, Ptr{IntSxp}), sexpnum(S), sdims)
end
...
function findVarInFrame(e::Ptr{EnvSxp}, s::Ptr{SymSxp})
    ccall(:Rf_findVarInFrame, Ptr{UnknownSxp}, (Ptr{EnvSxp}, Ptr{SymSxp}), e, s)
end
findVarInFrame(e, s) = findVarInFrame(sexp(e), sexp(SymSxp, s))

function defineVar(s::Ptr{SymSxp}, v::Ptr{S}, e::Ptr{EnvSxp}) where {S<:Sxp}
    ccall(:Rf_defineVar, Nothing, (Ptr{SymSxp}, Ptr{S}, Ptr{EnvSxp}), s, v, e)
    nothing
end
...
function newEnvironment(env::Ptr{EnvSxp})
    ccall(
        :Rf_NewEnvironment,
        Ptr{EnvSxp},
        (Ptr{NilSxp}, Ptr{NilSxp}, Ptr{EnvSxp}),
        sexp(Const.NilValue),
        sexp(Const.Const.NilValue),
        env,
    )
end
...
function findNamespace(str::String)
    ccall(:R_FindNamespace, Ptr{EnvSxp}, (Ptr{StrSxp},), sexp(str))
end
